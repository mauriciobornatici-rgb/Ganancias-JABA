...
import {
  ...
  type SaleCategoryBreakdown,
  type SalesMonthFilter,
  type SalesMonthlySummary,
} from '@/domain/ganancias/presentation/salesMonthlySummary';

type SalesMonthlySummaryPanelProps = {
  summary: SalesMonthlySummary;
  activeFilter: SalesMonthFilter;
  activeFilterLabel: string;
  onFilterChange: (filter: SalesMonthFilter) => void;
};

/** Renglones por categoría de venta dentro de una tarjeta (solo categorías con movimiento). */
function SaleCategoryLines({ byCategory }: { byCategory: SaleCategoryBreakdown }) {
  const lines = listSaleCategoryBreakdown(byCategory);
...
export function SalesMonthlySummaryPanel({
  summary,
  activeFilter,
  activeFilterLabel,
  onFilterChange,
}: SalesMonthlySummaryPanelProps) {
  return (
    <section
      aria-label="Resumen y filtro mensual de comprobantes de ventas"
      className="rounded-xl border border-zinc-800 bg-[#0b0b0e] p-4 space-y-3"
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
          <h3 className="text-xs font-extrabold uppercase tracking-[0.14em] text-zinc-200">Ventas por mes</h3>
          <p className="mt-1 text-[10px] text-zinc-500">Neto gravado computable y cantidad de comprobantes (exentas y no computables no suman). Seleccione un mes para analizar su detalle.</p>
        </div>
        <button
          type="button"
        ...
        >
          <span className="block text-[9px] font-black uppercase tracking-wider text-teal-300">Ver todos los meses</span>
          ...
          <span className="mt-0.5 block text-[9px] text-zinc-500">
            {listSaleCategoryBreakdown(summary.totalByCategory)
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
        {summary.months.map(month => {
          ...
          return (
            ...
            >
              ...
              <SaleCategoryLines byCategory={month.byCategory} />
            </button>
          );
        })}
        ...
        {summary.undated.count > 0 && (
          ...
          >
            ...
            <SaleCategoryLines byCategory={summary.undated.byCategory} />
          </button>
        )}
      </div>
