...
const TEST_SHADOW_DATABASE_NAME = 'ganancias_jaba_test_shadow';
 ...
 * @param {Record<string, string | undefined>} [env]
 */
export function resolveTestShadowDatabaseUrl(env = process.env) {
  const port = env.JABA_TEST_DB_PORT ?? DEFAULT_TEST_PORT;
  const candidate = env.TEST_SHADOW_DATABASE_URL ?? (
    `mysql://${TEST_DATABASE_USER}:${TEST_DATABASE_PASSWORD}@127.0.0.1:${port}/${TEST_SHADOW_DATABASE_NAME}`
  );
  const parsed = new URL(candidate);

  if (parsed.hostname !== '127.0.0.1' || parsed.pathname !== `/${TEST_SHADOW_DATABASE_NAME}`) {
    throw new Error('La base shadow de pruebas debe ser local y llamarse ganancias_jaba_test_shadow.');
  }

  if (parsed.username !== TEST_DATABASE_USER || parsed.password !== TEST_DATABASE_PASSWORD) {
    throw new Error('La base shadow de pruebas debe usar las credenciales Docker jaba_test.');
  }
